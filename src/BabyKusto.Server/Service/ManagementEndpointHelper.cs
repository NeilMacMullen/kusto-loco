...
public class ManagementEndpointHelper
{
    ...
    public KustoApiResult Process(KustoApiMgmtRequestBody body)
    {
        ...
        if (diagnostics.Count > 0)
        {
            ...
            {
                Console.WriteLine(
                    $"Kusto diagnostics: {diag.Severity} {diag.Code} {diag.Message} {diag.Description}");
            }
        ...
        {
            throw new InvalidOperationException(
                $"Expected a command block, found {code.Syntax.GetType().FullName}.");
        }

        if (commandBlock.Statements.Count != 1)
        {
            throw new InvalidOperationException(
                $"Espected a single statement in command block, found {commandBlock.Statements.Count}.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Expected an expression statement, found {statement.GetType().FullName}.");
        }
    ...
    private KustoApiResult ProcessStatement(KustoApiMgmtRequestBody body, ExpressionStatement statement)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Expected a custom command expression, found {statement.Expression.GetType().FullName}.");
        }
        ...
        if (command == EngineCommands.ShowClusterDatabases)
        {
            return ProcessShowClusterDatabasesCommand();
        }

        if (command == EngineCommands.ShowSchema)
        {
            return ProcessShowSchemaCommand();
        }

        if (command == EngineCommands.ShowDatabasesSchemaAsJson)
        {
            return ProcessShowSchemaCommand();
        }

        if (command == EngineCommands.CancelQuery)
        {
    ...
    private KustoApiResult ProcessShowVersionCommand()
    {
        ...
        result.Tables.Add(
            new KustoApiTableResult
            {
                ...
                Rows =
                {
                    new JsonArray(JsonValue.Create("0.0.0"), JsonValue.Create("2022-10-07T23:00:00Z"),
                        JsonValue.Create("Engine"), JsonValue.Create("0.0.0"),
                        JsonValue.Create("{\"Type\":\"Azure Data Explorer\"}")),
                },
            });
        return result;
    }
