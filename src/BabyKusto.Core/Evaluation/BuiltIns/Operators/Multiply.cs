...
namespace BabyKusto.Core.Evaluation.BuiltIns.Impl
{
    ...
    internal class MultiplyLongTimeSpanOperatorImpl : IScalarFunctionImpl
    {
        ...
        {
            Debug.Assert(arguments.Length == 2);
            return new ScalarResult(ScalarTypes.TimeSpan, TimeSpan.FromTicks((long)arguments[0].Value * ((TimeSpan)arguments[1].Value).Ticks));
        }
        ...
        public ColumnarResult InvokeColumnar(ColumnarResult[] arguments)
        {
            ...
            for (int i = 0; i < left.RowCount; i++)
            {
                data[i] = TimeSpan.FromTicks(left[i] * right[i].Ticks);
            }
