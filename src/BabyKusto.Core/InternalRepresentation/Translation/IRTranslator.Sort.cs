            ...
            foreach (var expression in node.Expressions)
            {
                ...
                if (expression.Element is OrderedExpression orderedExpression)
                {
                    ...
                    sortDirection = ordering.AscOrDescKeyword?.Kind switch
                    {
                        ...
                        _ => throw new InvalidOperationException($"Unexpected ordering kind {ordering.AscOrDescKeyword.Kind}"),
                    };
                    nullsDirection = ordering.NullsClause?.FirstOrLastKeyword.Kind switch
                    {
                        null => (sortDirection == SortDirections.Asc || sortDirection == SortDirections.GrannyAsc) ? NullsDirections.First : NullsDirections.Last,
                        SyntaxKind.FirstKeyword => NullsDirections.First,
                        SyntaxKind.LastKeyword => NullsDirections.Last,
                        _ => throw new InvalidOperationException($"Unexpected nulls ordering kind {ordering.NullsClause.Kind}"),
                    };
