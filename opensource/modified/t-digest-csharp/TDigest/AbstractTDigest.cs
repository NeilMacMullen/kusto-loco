...
public abstract class AbstractTDigest : Digest
{
     ...
     * <code>x2</code>
     * and is guaranteed to return a number between
     * <code>x1</code>
     * and
     * <code>x2</code>
     * .
    ...
    private static double Interpolate(double x, double x0, double x1) => (x - x0) / (x1 - x0);

    private static void Encode(byte[] buf, int n)
    {
        var k = 0;
        while (n < 0 || n > 0x7f)
        {
    ...
    private static int Decode(byte[] buf)
    {
        var i = 0;
        int v = buf[i++];
        var z = 0x7f & v;
     ...
     */
    private static double Quantile(double index, double previousIndex, double nextIndex, double previousMean,
        double nextMean)
    {
    ...
    public override bool IsRecording() => IsRecordingAllData;
    ...
    protected Centroid CreateCentroid(double mean, int id) => new(mean, id, IsRecordingAllData);
